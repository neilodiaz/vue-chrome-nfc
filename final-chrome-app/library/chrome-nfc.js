    ...
    for (var i = 0; i < this.ndef.length; i++) {
        ...
        switch (entry["type"]) {
            ...
            case "Text":
                arr.push({
                    "TNF": 1,
                    "TYPE": new Uint8Array([84]),
                    "PAYLOAD": this.compose_RTD_TEXT(entry["lang"], entry["text"])
                });
                break;
            case "URI":
...
NDEF.prototype.parse_RTD_URI = function (rtd_uri) {
    return {
        "type": "URI",
        "uri": this.prepending[rtd_uri[0]] + UTIL_BytesToString(rtd_uri.subarray(1, rtd_uri.length))
    };
};
        ...
        "findDevices": function (cb) {
            ...
            wait_for_passive_target(device, function (rc, tag_type, tag_id) {
                ...
                tag.read(device, function (rc, ndef) {
                    ...
                    // Add NFC Card Tag ID on the returned objectif
                    if (tag.tag_id) {
                        ndef_obj.ndef[0]["tag_id"] = UTIL_BytesToHex(tag.tag_id);
                    }

                    // console.log('ndef_obj: ',ndef_obj);
                    // console.log('tag_obj: ',UTIL_BytesToHex(tag.tag_id));
                    // console.log('---TAG ID---: ', UTIL_BytesToHex(new Uint8Array(tag_id)));
...
function usbSCL3711() {
    ...
    this.strerror = function (errno) {
        var err = {
            ...
            4: "erroneous bit count in anticollision",
            5: "framing error during mifare operation",
            6: "abnormal bit collision in 106 kbps anticollision",
            7: "insufficient communication buffer size",
            9: "rf buffer overflow detected by ciu",
            10: "rf field not activated in time by active mode peer",
            11: "protocol error during rf communication",
            13: "overheated - antenna drivers deactivated",
            14: "internal buffer overflow",
            16: "invalid command parameter",
            18: "unsupported command from initiator",
            19: "format error during rf communication",
            20: "mifare authentication error",
            24: "not support NFC secure",
            25: "i2c bus line is busy",
            35: "wrong uid check byte (14443-3)",
            37: "command invalid in current dep state",
            38: "operation not allowed in this configuration",
            39: "not acceptable command due to context",
            41: "released by initiator while operating as target",
            42: "card ID does not match",
            43: "the card previously activated has disapperaed",
            44: "Mismatch between NFCID3 initiator and target in DEP 212/424 kbps",
            45: "Over-current event has been detected",
            46: "NAD missing in DEP frame",
            47: "deselected by initiator while operating as target",
            49: "initiator rf-off state detected in passive mode",
            127: "pn53x application level error"
        };
        if (errno in err) {
